...
<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Men's Shoes</h1> 

    {{-- Product Grid: Using lg:grid-cols-4 for better balance with fewer items --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-md overflow-hidden flex flex-col transition duration-300 hover:shadow-xl hover:border-gray-300">
            ...
            {{-- Image Link --}}
            <a href="#">
               <img class="w-full h-56 object-cover" {{-- Slightly reduced image height for better balance --}}
                    src="data:image/jpeg;base64,{{ base64_encode($product->image) }}"
                    alt="{{ $product->title }}" />
            </a>

            <div class="p-4 flex flex-col flex-grow"> {{-- Reduced card padding --}}
                
                {{-- Title & Category --}}
                <a href="#" class="flex-grow">
                    <h5 class="text-lg font-semibold tracking-tight text-gray-900 hover:text-gray-700 transition line-clamp-2"> {{-- Reduced title size slightly --}}
                        {{ $product->title }}
                    </h5>
                </a>
                ...
                {{-- Description (Removed for cleaner look on small cards, or keep it short) --}}
                <p class="text-sm text-gray-600 mt-2 mb-2 line-clamp-2 hidden sm:block">
                    {{-- Optional: Keep description only on medium/large screens --}}
                    {{ $product->description }}
                </p>
                ...
                <div class="flex items-center justify-between mt-3 border-t border-gray-100 pt-3">
                    <span class="text-xl font-extrabold text-gray-900"> {{-- Reduced price font size --}}
